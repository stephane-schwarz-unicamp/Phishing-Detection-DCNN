...
#dataframe = pd.read_csv("./data/mediaeval/twitter_posts.csv")
dataframe = pd.read_csv('./data/phishing/filtered_dataset.csv')
...
documents = posts.tolist()

features = [one_hot(document, 5000) for document in documents]

features = pad_sequences(features, maxlen=58, padding='post', value=15448)

from sklearn.preprocessing import OneHotEncoder, LabelEncoder

le = LabelEncoder()
encoded_labels = le.fit(labels).transform(labels)

X_train, X_val, y_train, y_val = train_test_split(features, encoded_labels, test_size=0.2)
...
train_x_indexes, train_y, train_lengths = dataUtils.read_and_sort_matlab_data(X_train, y_train)

dev_x_indexes, dev_y, dev_lengths = dataUtils.read_and_sort_matlab_data(X_val, y_val)

test_x_indexes, test_y, test_lengths = dataUtils.read_and_sort_matlab_data(X_test, y_test)

# print train_x_indexes.shape, train_y.shape, train_lengths[-1]

# print dev_x_indexes.shape, dev_y.shape, len(dev_lengths)

# print test_x_indexes.shape, test_y.shape, len(test_lengths)
...
while (epoch < hyperparas['n_epochs']):
    ...
    for minibatch_index in permutation:
        ...
        if batch_counter>0 and batch_counter % hyperparas["valid_freq"] == 0:
            ...
            for minibatch_dev_index in range(n_dev_batches):
                ...
                train_acc_value = valid_model(x_input,y_input)

                # accuracy_valid.append(valid_model(x_input,y_input))
                accuracy_valid.append(train_acc_value)
            ...
            if this_validation_accuracy > best_validation_accuracy:
                ...
                for minibatch_test_index in range(n_test_batches):
                    ...
                    test_acc_value = test_model(x_input,y_input)
                    accuracy_test.append(test_acc_value)
    ...
    X_plot_val.append(train_acc_value)
    X_plot_test.append(test_acc_value)
    y_plot_val.append(epoch)
    y_plot_test.append(epoch)

fig = plt.figure()
ax = plt.axes()

# plt.plot(X_plot_train, y_plot_train, color='-b')
plt.plot(X_plot_val, y_plot_val, color='-g')   
plt.plot(X_plot_test, y_plot_test, color='-m') 

# # plt.plot(X_plot_train_loss, y_plot_train_loss, color='--r')
# plt.plot(X_plot_val_loss, y_plot_val_loss, color='--c')   
# plt.plot(X_plot_test_loss, y_plot_test_loss, color='--k')

plt.savefig('./dcnn_fake_news_detection.png')
